...
fun updateWidget(context: Context, views: RemoteViews)
{
    ...
    val schedule = MainActivity.schedule?.value
    val schoolLessons = MainActivity.schoolLessons?.value

    if (schedule != null && schoolLessons != null) {
        ...
        for (scheduleBlock in scheduleBlocks)
        {
            ...
            if (startDeltaTime > 0)
            {
                ...
                if (schoolLesson != null)
                {
                    ...
                    val widgetLessonStartingString = context.getString(R.string.widget_lesson_starting)

                    text = when (language) {
                        "pt" -> widgetLessonStartingString.format(schoolLesson.shortName, time, timeName)
                        else -> widgetLessonStartingString.format(schoolLesson.shortName, time, timeName)
                    }
                }
                break
            }
                    ...
                    if (schoolLesson != null)
                    {
                        ...
                        text = when (language) {
                            "pt" -> if (endDeltaTime.hour == 0)
                                context.getString(R.string.widget_lesson_ending_minutes).format(faltar, endDeltaTime.minute, timeName, schoolLesson.shortName)
                            else
                                context.getString(R.string.widget_lesson_ending_hours).format(faltar, endDeltaTime.hour, endDeltaTime.minute, schoolLesson.shortName)
                            else -> if (endDeltaTime.hour == 0)
                                context.getString(R.string.widget_lesson_ending_minutes).format(endDeltaTime.minute, timeName, schoolLesson.shortName)
                            else
                                context.getString(R.string.widget_lesson_ending_hours).format(endDeltaTime.hour, endDeltaTime.minute, schoolLesson.shortName)
                        }
                    }
                    break
                }
